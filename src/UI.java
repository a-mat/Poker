		 ...
		 rootNode2.add(joeStatus,45,6);
		 		...
		 		public void handle(ActionEvent ae){
					...
					rootNode2.getChildren().remove(betText);
					 computerAi();
						 ...
						 potLabel.setText("Pot: "+String.valueOf(GameController.getBettingPool()));
						 if(commCheck==1){
							 communityCard.setText(String.valueOf(GameController.theFlop()).replace("[","")
									 .replace("]",""));
						 ...
						 if(commCheck==2){
							 communityCard.setText(String.valueOf(GameController.theTurn()).replace("[","")
									 .replace("]",""));
								rootNode2.add(betBtn,25,14);
					 			rootNode2.add(checkBtn,30,14);
					 			rootNode2.add(foldBtn,35,14);
						 ...
						 if(commCheck==3){
							 communityCard.setText(String.valueOf(GameController.theRiver()).replace("[","")
									 .replace("]",""));
								rootNode2.add(betBtn,25,14);
					 			rootNode2.add(checkBtn,30,14);
					 			rootNode2.add(foldBtn,35,14);
						 ...
						 if(commCheck==4){
							 ...
							 for(Player user: GameController.getAllUsers()){
								 if(user.isPlayerStatus()==true){
									 if(user.getName().equals("Player")) playaStatus.setText("Player had"
											 +HandEvaluator.checkHand(HandEvaluator.loadCards(user.getHand())));
									 if(user.getName().equals("June")) juneStatus.setText("June had"
											 +HandEvaluator.checkHand(HandEvaluator.loadCards(user.getHand())));
									 if(user.getName().equals("Aaron")) aaronStatus.setText("Aaron had"
											 +HandEvaluator.checkHand(HandEvaluator.loadCards(user.getHand())));
									 if(user.getName().equals("Joe")) joeStatus.setText("Joe had"
											 +HandEvaluator.checkHand(HandEvaluator.loadCards(user.getHand())));
		  ...
		  raiseBtn.setOnAction(new EventHandler<ActionEvent>(){
		 		public void handle(ActionEvent ae){
		 			...
		 			if(GameController.raiseCounter>=3) bottomInfo.setText("Raise cant be used twice");
		 			else {rootNode2.getChildren().remove(raiseBtn);
		  ...
		  raiseText.setOnAction(new EventHandler<ActionEvent>(){
				...
				for(int i=1;i<GameController.getAllUsers().size();i++){
					 if(i==2) {
						 ...
						 round(GameController.getAllUsers().get(i),2,3);  //make alhorithim
						 System.out.println(GameController.getAllUsers().get(i)+"called");
						 GameController.getAllUsers().get(i).setUserCurrentBet(GameController.getCurrentBet());
					 }
					 juneCash.setText(String.valueOf( GameController.getAllUsers().get(1).getCash()));
	 ...
	 void computerAi(){
		 ...
		 for(int i=1;i<GameController.getAllUsers().size();i++){
			 if(GameController.raiseCounter>2){
				 ...
				 if(chance<90) choice=2;
				 if(chance>=90) choice=1;
			 }
			 else{
				 System.out.println(" raise");
				 chance = (int) (Math.random()*101);
				 if(chance<75) choice=2;
				 if(chance>=75 && chance<95) {
					 choice=0;
					 if(GameController.getCurrentBet()<=500) raiseChoice=(int) ((GameController.getCurrentBet())*.90);
					 if(GameController.getCurrentBet()>500 && GameController.getCurrentBet()<1000)
						 raiseChoice=(int) ((GameController.getCurrentBet())*.50);
					 if(GameController.getCurrentBet()>=1000) raiseChoice=(int) ((GameController.getCurrentBet())*.20);
				 }
				if(chance>=95) choice=1;
